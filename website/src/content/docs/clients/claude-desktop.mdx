...
<Figure
  alt="claude_desktop_config.json open in an editor with the mcpServers block visible"
  caption="Claude Desktop config file location"
/>
...
the MCP server needs `JINA_API_KEY` at launch too - add an `env` block:
...
`RENDERER_UNAVAILABLE` error and the agent can't fetch page bodies.
